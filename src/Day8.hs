...
import Helpers (splitOn,inCommon,sort)
...
day8 p2 x = let input = map (map words.splitOn"|") $ lines x in if p2 then getNum input else length . filter (`elem`[2,3,4,7]) $ concatMap (map length . (!!1)) input